...
async function getLocalUserForLogtoToken(token: string, sub: string, payloadEmail?: string) {
...
export const jwt = async (req, res, next) => {
  ...
  try {
    ...
    const localUser = await getLocalUserForLogtoToken(token, sub, claimEmail)
    if (localUser) {
