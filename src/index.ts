...
program
  ...
  .action((scanPath: string | undefined, opts: { json?: boolean; severity?: string; rules?: string }) => {
    const dir = scanPath ?? '.';
    ...
    const activeRules = rules.filter(r => !ruleFilter || ruleFilter.includes(r.id));
...
program
  ...
  .action((opts: { json?: boolean }) => {
    if (opts.json) {
      console.log(JSON.stringify(rules.map(r => ({ id: r.id, severity: r.severity, title: r.title, description: r.description })), null, 2));
      return;
    }
